...
- EXIF-GPS via `lib/exif-utils.ts readExifGps()` (importiert die `exifr` mini bundle deep). WICHTIG: vor `resizeImage()` aufrufen — Canvas-Resize strippt EXIF.
- `<PinPickerMap>` (in `components/PinPickerMapClient.tsx` als ssr:false-wrapper) für single tappable/draggable Pin auf Karte. Reuse `buildPinSvg(type)` + ICON_CACHE-Pattern.
- Live-Tracking-State auf SpotMap ist opt-in pro Session (kein localStorage), accuracy-Filter <200m für updates, <80m für initial-lock. `liveTracking` und `onLiveUpdate` werden via `useRef` aus dem watchPosition-Callback gelesen — verhindert Effect-Teardown auf Toggle. `followBroken` wird via dragstart+zoomstart in FollowController gesetzt.
