...
**Current state: Phase 8.6 Performance complete (v0.8.6).** Marketing-stats helpers (getSpotCounts/getLivingNumbers/getRecentActivity/getHeroSampleSpots) sind in `unstable_cache` gewrappt mit `revalidate: 60` und Tag `marketing-stats` — landing-page TTFB drastisch besser. Mutations (actions/spots.ts, actions/admin.ts adminDeleteSpot) nutzen `updateTag('marketing-stats')` für read-your-own-writes invalidation. Loading-Skeletons in `app/(marketing)/loading.tsx`, `app/(app)/map/loading.tsx`, `app/(app)/admin/loading.tsx`. Spot-Foto in SpotDetail nutzt `next/image` mit responsive `sizes` und `priority`. `next.config.ts` `images.remotePatterns` für Supabase Storage. Bundle-analyzer via `npm run analyze` (ANALYZE=true env). Cookie-Banner-Spec deferred. Map-Marker bleiben Drop-Pins. Next: Phase 9 (Notifications/Email/Public-Profile/etc.) oder weiterer Polish nach Beta-Feedback.
