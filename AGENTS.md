...
**Current state: Phase 8.4 Admin-Polish + Emoji-Cleanup complete (v0.8.3).** Admin RLS-Bug ist gefixt (createAdminClient für Spot-Queries), Stats-Overview + Search/Filter + User-Detail (`/admin/users/[id]`) + Moderation (`/admin/moderation`) sind live, alle UI-Emojis durch Tabler-Icons ersetzt (Pfeile/Häkchen/Profil-Menü etc.; Map-Marker bleiben Emoji bis Phase 8.3.1 mit SVG-DivIcon-Refactor; EmojiPicker + StarPicker bleiben Emoji-basiert). Cookie-Banner-Spec liegt unter `docs/superpowers/specs/2026-05-06-cookie-banner-spec.md` (deferred bis Tracking eingeführt wird). Next: Phase 8.3.1 (Map-Marker SVG-DivIcon-Refactor), Phase 9 (Notifications/Public Profile/etc.).
...
- UI-Emojis sind out — alle in 8.3-8.4 durch `@tabler/icons-react` ersetzt. Ausnahmen: Map-Marker (deferred 8.3.1), EmojiPicker (literal user-marker selection), StarPicker ⭐ (opacity-fill UX-Pattern), `SPOT_TYPES.emoji` und `SPOT_VISIBILITIES.emoji` Felder (Daten-Stopgap, nicht UI-Render).
