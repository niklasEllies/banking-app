...
**Current state: Phase 8.3.1 + 8.5 complete (v0.8.4).** Map-Marker sind jetzt Drop-Pins (lib/spot-marker-svg.ts: hardcoded Tabler-Icon-Paths in pin-shape SVG, kein react-dom/server overhead). Cluster zeigt Zahlen-Pin. SEO/OG live: lib/site.ts mit SITE_URL/SITE_NAME, app/sitemap.ts dynamic mit public spots, app/robots.ts, app/(marketing)/opengraph-image.tsx mit dynamic ImageResponse (1200×630 Topo-Forest-Deep). Set `NEXT_PUBLIC_SITE_URL` in Vercel for canonical URLs. Cookie-Banner-Spec deferred bis Tracking. EmojiPicker + StarPicker + ThemeToggle + GPS-Banner ⚠️ bleiben Emoji-basiert (intentional). Next: Phase 9 (Notifications/Email/Public-Profile/Friend-Activity-Feed/Web-Push).
...
- UI-Emojis sind out — alle in 8.3-8.4 durch `@tabler/icons-react` ersetzt. Map-Marker seit 8.3.1 sind Drop-Pin-SVGs via `lib/spot-marker-svg.ts` (Tabler-Icon-Paths hardcoded — beim Tabler-Update gegenchecken). Ausnahmen: EmojiPicker (literal user-marker selection), StarPicker ⭐ (opacity-fill UX-Pattern), ThemeToggle ☀️🌙🌓 (Theme als Glyph), GPS-Banner ⚠️ (semantic), `SPOT_TYPES.emoji` und `SPOT_VISIBILITIES.emoji` Felder (Daten-Stopgap, nicht UI-Render — bleiben für Fallbacks).
